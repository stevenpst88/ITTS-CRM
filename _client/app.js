...
function setActiveNav(section) {
  ['navHome','navManagerHome','navProspects','navContacts','navCompanyMaster','navKeyAccount','navVisits','navTargets','navPipeline','navForecast','navLostOpp','navExecDash','navYoy','navCampaigns','navLeads','navQuotations','navPipelineReport','navBizAnalysis','navErpMa','navSapMa','navReceivables','navCallin','navTransfer'].forEach(id => { const el=$(id); if(el) el.classList.remove('active'); });
  const map = { null:'navHome', managerHome:'navManagerHome', prospects:'navProspects', contacts:'navContacts', companyMaster:'navCompanyMaster', keyAccount:'navKeyAccount', visits:'navVisits', targets:'navTargets', pipeline:'navPipeline', forecast:'navForecast', lostOpp:'navLostOpp', execDash:'navExecDash', yoy:'navYoy', campaigns:'navCampaigns', leads:'navLeads', quotations:'navQuotations', pipelineReport:'navPipelineReport', bizAnalysis:'navBizAnalysis', 'erp-ma':'navErpMa', 'sap-ma':'navSapMa', receivables:'navReceivables', callin:'navCallin', transfer:'navTransfer' };
  const el = $(map[section]);
  if (el) el.classList.add('active');
  const titles = { null:'首頁', managerHome:'主管首頁', prospects:'潛在客戶', contacts:'我的客戶', companyMaster:'企業主檔', keyAccount:'Key Account', visits:'業務日報', targets:'業務年度目標', pipeline:'商機推進進度', forecast:'銷售預測報表', lostOpp:'流失商機分析', execDash:'管理儀表板', yoy:'YoY 營收同期比', campaigns:'行銷活動', leads:'Lead 管理', quotations:'報價單管理', pipelineReport:'商機動態報表', bizAnalysis:'商機分析', 'erp-ma':'ERP MA 合約管理', 'sap-ma':'SAP License MA 管理', receivables:'應收帳款逾期', callin:'Call-in Pass 管理', transfer:'責任業務名單移轉' };
...
function cmShowImportPreview(d) {
  ...
  let html = `<div style="margin-bottom:12px">`
    ...
    + (d.noTaxId ? chip('無統編', d.noTaxId, '#ea580c') : '')
  ...
  if (d.noTaxId > 0) {
    html += `<div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:10px 12px;margin-bottom:10px;color:#9a3412;font-size:13px">🌐 偵測到 <b>${d.noTaxId}</b> 筆<b>無統一編號</b>的新公司（如國外客戶）。將以<b>公司名</b>比對（無法用統編去重，日後同名易產生重複）。確認後仍會照常匯入。</div>`;
    html += `<div style="overflow:auto;max-height:160px;border:1px solid #eee;border-radius:8px;margin-bottom:12px"><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="background:#fff7ed"><th style="padding:6px 8px;text-align:left;width:60px">列</th><th style="padding:6px 8px;text-align:left">公司名稱（無統編）</th></tr></thead><tbody>`;
    html += (d.noTaxIdSamples || []).map(s => `<tr style="border-top:1px solid #f0f0f0"><td style="padding:6px 8px;color:#6b7280">${s.rowNum}</td><td style="padding:6px 8px">${escapeHtml(s.name || '(空白公司名)')}</td></tr>`).join('');
    html += `</tbody></table></div>`;
    if (d.noTaxId > (d.noTaxIdSamples || []).length) html += `<div style="font-size:12px;color:#9ca3af;margin:-6px 0 12px">（僅顯示前 ${d.noTaxIdSamples.length} 筆，共 ${d.noTaxId} 筆）</div>`;
  }
...
function renderMonthBudgetCard() {
  ...
  const tables = budgetRows.map(b => {
    ...
    const totalRevPct = totalBudget > 0 ? Math.round(totalActual / totalBudget * 100) : 0;
          ...
          <table class="mb-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                ${gmBudgetCells}
                <td class="mb-td mb-total-col">${totalGmBudget > 0 ? totalGmBudget.toLocaleString() : '—'}</td>
              </tr>
              <tr>
...
function openMbActualModal(owner, year) {
  ...
  const existGmActuals = rec && rec.grossMarginActuals ? rec.grossMarginActuals : Array(12).fill(null);

  const buildInputs = (data, gridId) => {
    const g = $(gridId);
    if (!g) return;
  ...
  buildInputs(existActuals,   'mbActualGrid');
  buildInputs(existGmActuals, 'mbGmActualGrid');
...
function updateMbActualTotal() {
  const revInputs = document.querySelectorAll('#mbActualGrid input');
  const gmInputs  = document.querySelectorAll('#mbGmActualGrid input');
  ...
  const gmLbl = $('mbGmActualTotalLabel');
...
async function saveMbActuals() {
  const revInputs = document.querySelectorAll('#mbActualGrid input');
  const gmInputs  = document.querySelectorAll('#mbGmActualGrid input');
  const actuals = Array.from(revInputs).map(inp =>
    inp.value === '' ? null : parseFloat(inp.value)
  );
  const grossMarginActuals = Array.from(gmInputs).map(inp =>
    inp.value === '' ? null : parseFloat(inp.value)
  );
  try {
    const res = await fetch(`${API}/monthly-budget/actuals`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: _mbActualOwner, year: _mbActualYear, actuals, grossMarginActuals })
    });
...
function getGmActual(year, month, ownerFilter) {
  return allOpportunities
    ...
    .reduce(function(s, o) {
      ...
      var gmr = parseFloat(o.grossMarginRate) || 0;
      return s + amt * gmr / 100;
    }, 0);
}
