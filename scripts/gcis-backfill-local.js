...
(async () => {
  ...
  const selector = RECAP ? isRecapTarget : STATUS ? isStatusTarget : isTarget;
  ...
  if (!APPLY) {
    ...
    console.log(`    node scripts/gcis-backfill-local.js --apply${RECAP ? ' --recap' : ''}${STATUS ? ' --status' : ''}${argLimit ? ' --limit ' + argLimit : ''}`);
    await pg._poolEnd?.();
    process.exit(0);
  }
